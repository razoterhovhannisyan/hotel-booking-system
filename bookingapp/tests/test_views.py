...
class RoomListViewTest(APITestCase):
    ...
    def test_roomlisview_get(self):
        ...
        serializer_data = serializers.RoomListSerializer([self.room_1, self.room_2], many=True).data
...
class RoomDetailViewTest(APITestCase):
    ...
    def test_roomdetailview_get(self):
        self.client.credentials(HTTP_AUTHORIZATION = f'Bearer {self.access_token}')
        response = self.client.get(reverse('room-details', args = [self.room_1.id]))
        response_data = response.data
...
class GetBookingView(APITestCase):
    ...
    def test_get_bookings_auth(self):
        ...
        response_data = response.data
        serializer_data = serializers.BookingSerializer([self.booking_1], many=True).data
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class RoomFilterSortTest(APITestCase):
    ...
    def test_filter_min_price(self):
        ...
        filtered_rooms = booking_models.Room.objects.filter(cost_per_day__gte=150)
        expected_data = serializers.RoomListSerializer(filtered_rooms, many=True).data
    ...
    def test_filter_max_price(self):
        ...
        expected_data = serializers.RoomListSerializer(expected_rooms, many=True).data
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_sort_price(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        response = self.client.get(reverse('room-filter'), {'sort_by': 'price'})
    ...
    def test_sort_capacity(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        response = self.client.get(reverse('room-filter'), {'sort_by': 'capacity'})
...
class RoomAvailabilityTest(APITestCase):
    def setUp(self):
        ...
        self.room_1 = booking_models.Room.objects.create(number=101,cost_per_day=100,capacity=2)
        self.room_2 = booking_models.Room.objects.create(number=102,cost_per_day=150,capacity=4)
    ...
    def test_room_availability(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        start_date = timezone.make_aware(datetime(2024, 7, 28, 12, 0, 0)).isoformat()
        end_date = timezone.make_aware(datetime(2024, 7, 30, 12, 0, 0)).isoformat()
        response = self.client.get(reverse('availability-rooms'), {'start_date': start_date, 'end_date': end_date})
    ...
    def test_room_availability_no_rooms(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        start_date = timezone.make_aware(datetime(2024, 7, 20, 12, 0, 0)).isoformat()
        end_date = timezone.make_aware(datetime(2024, 7, 30, 12, 0, 0)).isoformat()
        response = self.client.get(reverse('availability-rooms'), {'start_date': start_date, 'end_date': end_date})
    ...
    def test_invalid_dates(self):
        self.client.credentials(HTTP_AUTHORIZATION=f'Bearer {self.access_token}')
        response = self.client.get(reverse('availability-rooms'), {'start_date':
        'invalid-date', 'end_date': 'invalid-date'})
